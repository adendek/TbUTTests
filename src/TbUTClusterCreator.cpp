...
ClusterCreator::ClusterCreator(bool& p_isPtype,ITresholdProvider& p_thresholds):
		m_isPtype(p_isPtype),
		m_clusterMaxSize(4),
...
ClusterContainer::ClusterVector ClusterCreator::createClusters(RawData *p_inputData)
{
...
void ClusterCreator::findCulsterSeeds(RawData *p_inputData)
{
	int l_channelNumber=RawData::getnChannelNumber();
	for(auto channel=0;channel<l_channelNumber;channel++){
		RawData::DataType l_channelSignal=p_inputData->getSignal(channel);
...
bool ClusterCreator::isBiggerThanSeedThreshold(RawData::DataType p_channelSignal, int p_channel) const
{
...
Cluster ClusterCreator::createCluster(RawData *p_inputData, int l_channel) const
{
	int l_seedSize=1;
	Cluster l_cluster;
...
void ClusterCreator::removeDuplicatedSeeds(RawData *p_inputData)
{
...
bool ClusterCreator::arePartOfTheSameCluster(RawData* p_inputData, ClusterIterator& p_firstIt, ClusterIterator& p_secondIt) const
{
...
void ClusterCreator::extendClusterSeeds(RawData *p_inputData)
{
...
void ClusterCreator::extendCluster(ClusterIterator& p_clusterIt,RawData* p_inputData)
{
	int l_actualShift=1;
...
bool ClusterCreator::isStripNeedToBeAddedToCluster(ClusterIterator& p_clusterIt, RawData *p_inputData, int p_stripShift,bool p_isCheckingLeft ) const
{
...
void ClusterCreator::updateCluster(ClusterIterator& p_clusterIt,RawData *p_inputData ,int p_stripShift, bool p_isCheckingLeft)
{
	int l_sign =(p_isCheckingLeft)? -1:1;
	int l_channelNumber=p_clusterIt->m_seedPosition+l_sign*p_stripShift;
...
bool ClusterCreator::isBiggerThanLowThreshold(RawData::DataType p_channelSignal, int p_chnnel) const
{
...
void ClusterCreator::fillOuterStrips(RawData *p_inputData)
{
	for(auto& cluster : m_culsterVector){
		...
		/// normalize position
		cluster.m_position/=cluster.m_charge;
	}
}
